...
if __name__ == "__main__":
	...
	parser.add_argument('--metric', type=str, default='Linf', help='type of perturbations')
	parser.add_argument('--epsilon', type=float, default=0.03137, help='perturbation strength')	
	parser.add_argument('--n_cluster', type=int, default=10, help='total number of cubes allowed')

	parser.add_argument('--delta', type=float, default=0.05, help='additional points to reduce overfitting')
	...
	print('metric: {metric}, ' 'epsilon: {epsilon}, ' '#cubes: {n_cluster}, ' 
	...
	for j in range(args.repeat):
		...
		neighbor_ind_arr, _ = knn_graph(train_data, k=args.k, method='ball-tree', metric='chebyshev')
		...
		#### place the cubes incrementally for the smallest expansion
		centroids = []
		edge_sizes = []
		index_init = []
		index_expand = []
		for t in range(args.n_cluster):
			...
			for i in range(n_train):
				...
				center = train_data[i]
				dist, _ = torch.max(torch.abs(train_data[ind_valid] - center), dim=1)
				dist_expand, _ = torch.max(torch.abs(train_data[ind_valid_expand] - center), dim=1)
				...
				for s in n_point_arr:
					...
					edge_size = torch.max(torch.abs(train_data[neighbor_ind[s+1]] - center)) - 1e-6		## reduce overfitting
					count_init = torch.sum(dist <= edge_size).double()
					count_expand = torch.sum(dist_expand <= edge_size + args.epsilon).double()				
					n_expand = count_expand - count_init

					## record the indices (initial coverage and expanded coverage)
					ind_init = ind_valid[torch.nonzero(dist <= edge_size).view(-1)]
					ind_expand_total = ind_valid[torch.nonzero(dist <= edge_size + args.epsilon).view(-1)]
					...
					if iter_count % args.verbose == 0:
						print('Progress: {prog:.2%}\t\t' 'Initial: {init}\t\t' 'Expanded: {expand}\t\t'
								'Uncertainty: {lu:.3f}\t\t' 'Edge Size: {edge_size:.2f}'.format(
								prog=float(iter_count) / float(n_train*len(n_point_arr)), init=len(ind_init), 
								expand=len(ind_expand), lu=lu_err_reg, edge_size=edge_size))
		...
		for idx, data in enumerate(test_data):
			...
			for num, center in enumerate(centroids):
				diff = torch.max(torch.abs(data-center))
				if diff <= edge_sizes[num]:
					count += 1
					err_inds.append(idx)
					break
			for num, center in enumerate(centroids):
				diff = torch.max(torch.abs(data-center))	
				if diff <= edge_sizes[num]+args.epsilon:
					count_expand += 1
					break
